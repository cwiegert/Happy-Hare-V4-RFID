...
# nfc_gates.log must be stubbed before importing rc522_driver so the
...
from nfc_gates.rc522_driver import (
    RC522Driver,
...
class MockSPI:
    ...
    def sent_to(self, reg):
        """Return all values written to reg."""
...
def _build_tag_sequence(uid=(0xA3, 0xF2, 0x00, 0xCC)):
    ...
    responses  = transceive_ok([0x00, 0x04])
...
def test_init_sends_reset_command():
    spi = MockSPI(transfer_responses=[
        ...
        _reg_read_resp(_TxControlReg, 0x03),
    ])
...
def test_tag_present_returns_uid():
    ...
    spi = MockSPI(transfer_responses=_build_tag_sequence(uid=(0xA3, 0xF2, 0x00, 0xCC)))
    driver = RC522Driver(spi, gate=0, transceive_delay=0.0)
...
def test_anticoll_checksum_failure_returns_none():
    ...
    responses  = transceive_ok([0x00, 0x04])
    responses += transceive_ok(list(uid) + [bad_chk])
...
def test_read_uses_correct_spi_format():
    ...
    spi = MockSPI(transfer_responses=[[0x00, 0x5A]])
    driver = RC522Driver(spi, gate=0)
