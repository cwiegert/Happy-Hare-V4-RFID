...
|---|:---:|---|
| `[nfc_gate shared]` config section with `shared: true` | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `shared: true` → `mmu_gate` not required (internal sentinel 255) | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `shared: true` → `scan_enabled` forced false | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| Only one shared reader permitted per config | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `NFC GATE=` mux command not registered for shared reader | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `NFC_SHARED` command — READ, STATUS, CLEAR, PRELOAD_CHECK, POLL, SCAN, INIT, CLEAR_CACHE | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| Pending spool state — uid, spool_id, deadline, auto_created | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `EVENT_CHANGED` stores pending spool, stops polling, fires LED | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `EVENT_REMOVED` keeps pending spool intact | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `EVENT_UID_ONLY` records error without clearing pending spool | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `shared_pending_timeout` — pending spool expires after N seconds | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `shared_read_timeout` — polling auto-stops if no tag resolves | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `startup_polling: 1` — polls at Klipper boot with no read deadline | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| Poll skip when printing | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| Poll skip when HH action is active (load / unload / homing) | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `PRELOAD_CHECK` stages `MMU_GATE_MAP NEXT_SPOOLID=<id>` | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| Auto-restart polling after successful `PRELOAD_CHECK` (no read deadline) | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| LED feedback via named `shared_tag_read_effect` (`mmu_led_effect`) | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `shared_status_line` shown in `NFC_SHARED STATUS` and `NFC_STATUS` | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `_delayed_init` ready message shows `NFC_SHARED READ=1`, no HH seed note | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| Debug levels 0–4 throughout shared path | ✅ | `klippy/extras/nfc_gates/nfc_manager.py` |
| `_NFC_SHARED_PRELOAD` macro — HH pre-load hook bridge | ✅ | `config/nfc_macros.cfg` |
| `_NFC_SHARED_POST_UNLOAD` macro — HH post-unload hook bridge | ✅ | `config/nfc_macros.cfg` |
| Installer — lane / shared upfront branch (lane default) | ✅ | `install.sh` |
| Installer — `detect_reader_type` and `detect_shared_mcu` for re-install defaults | ✅ | `install.sh` |
| Installer — `write_shared_config` generates `[nfc_gate shared]` section | ✅ | `install.sh` |
| Installer — shared summary and next steps (MCU flash, HH hook wiring) | ✅ | `install.sh` |
| Minimize Klipper reactor blocking — `sleep_fn` → `reactor.pause` in PN532 driver | ✅ | `klippy/extras/nfc_gates/pn532_driver.py`, `nfc_manager.py` |
| Installer — generated shared config should also show `shared_read_timeout` alongside `shared_pending_timeout` and `shared_tag_read_effect` | ❌ | `install.sh` |
| Direct shared-reader tests — config parsing, singleton validation, `NFC_SHARED` commands, pending timeout, no-op preload, `NEXT_SPOOLID` staging, status output | ❌ | `tests/` |
| PN532 low-level debug sleep exception — normal driver waits use `sleep_fn`, but low-level debug helper still has direct `time.sleep()` calls | ❌ | `klippy/extras/nfc_gates/pn532_driver.py` |
| Documentation consistency — lane-only wiring/architecture docs need a shared-reader exception so they do not imply shared installs are unsupported | ❌ | `docs/i2c-pn532/wiring.md`, `docs/shared/architecture-decisions.md` |
| Hardware validation — verify HH post-unload timing starts shared polling after the gate is actually empty | ❌ | Happy Hare runtime, printer hardware |
| Hardware validation — verify `MMU_SET_LED EXIT_EFFECT=<shared_tag_read_effect> DURATION=3` plays the intended blinking yellow `mmu_RFID_read` effect | ❌ | Happy Hare runtime, LED config |
| Hardware validation — verify `MMU_GATE_MAP NEXT_SPOOLID=<id>` accepts auto-created spool IDs without `MMU_SPOOLMAN REFRESH=1` | ❌ | Happy Hare runtime, Spoolman |
