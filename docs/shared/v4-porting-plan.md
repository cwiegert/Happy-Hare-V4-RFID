...
- [x] ~~Fix or remove `NFCGate._has_per_lane_readers` (§7, found 2026-07-14)~~ — **fixed 2026-07-14.** Now computed in `_handle_connect()` from the module-level `_lane_instances` registry, once per instance, after all `[nfc_gate ...]` sections have loaded. See §7 for detail.
...
**✅ Fixed 2026-07-14 — `_has_per_lane_readers` now actually populated.** Was dead (see below for the original finding); now computed in `_handle_connect()` — `any(not shared and enabled for g in _lane_instances)` — which runs once per instance on `klippy:connect`, after every `[nfc_gate ...]` section has finished loading, so `_lane_instances` is stable by then. §4's action item closed.

**✅ Done 2026-07-14 — shared reader extracted into its own class, `SharedNFCReader(NFCGate)` in new `klippy/extras/nfc_gates/shared_reader.py`.** All ~38 `_shared_*`-named methods, plus `cmd_NFC_SHARED` and `_read_mmu_pending_timeout` (shared-only despite the name), moved off `NFCGate` onto the subclass; `get_status()` split into a lane-only base version and a `SharedNFCReader` override that merges in the shared-only fields (`pending_spool_id`, `preload_spool_id`, `has_per_lane_readers`, etc). `klippy/extras/nfc_gate.py`'s `load_config_prefix()` now picks the class from `config.getboolean('shared', False)` before construction, instead of one constructor branching internally.

Deliberately **not** touched: `_poll_timer_event()` and `_poll()` — both still live on `NFCGate` with `if self._shared:` branches baked into their control flow, inherited unmodified by `SharedNFCReader`. Splitting those is separate, larger design work (see the "where does the poll timer sit" discussion in the implementation log) — this pass only moved what was already self-contained. Same reasoning for `__init__`/`_handle_connect`: both stay on the base class, still branching on `self._shared` internally; `SharedNFCReader` just inherits them (`self._shared` still evaluates `True` for a `SharedNFCReader` instance since the constructor still reads it from config the same way).

`shared_preload.py` needed **zero** changes — `SharedPreloadCoordinator` already took `gate` as a duck-typed constructor argument, so it works identically whether `gate` is a plain `NFCGate` with `_shared=True` (before) or a `SharedNFCReader` instance (now). Module-level registry (`_lane_instances`, `_shared_instance`) and Klipper's `printer['nfc_gate shared']` object-name resolution needed no changes either — both are keyed by config section name / list membership, not Python class, and no `isinstance(x, NFCGate)` checks exist anywhere in the tree (confirmed by grep) that a subclass could break.

Also fixed as a byproduct of the move: a duplicate `shared_summary_line()` definition (the class body defined it twice; Python silently used only the second, so the first was dead code) is now just the one live version. `nfc_manager.py` shrank from 3595 to 2795 lines; `shared_reader.py` is 867 lines. `py_compile`, an AST-based undefined-name sweep, and `pyflakes` are clean on all three touched files (two pre-existing, unrelated warnings survive the move unchanged: an unused `last_action` local in `_shared_next_action()`, an unused `name` local in `nfc_gate.py`'s `load_config_prefix()` — both present before this session).

**Still open, deliberately not resolved this pass:** the `_poll_timer_event()`/`_poll()` split itself (six `if self._shared:` branch points, one of them implicit via `_scan_enabled` being forced `False` at config time rather than checked directly) — the user is spending more time on this specific design question before it's touched.

**Original finding, now fixed above (kept for context) — `_has_per_lane_readers` appears to be dead/never-populated.** `NFCGate._has_per_lane_readers` is initialized `False` in two places (`nfc_manager.py`) but grepping the whole file finds no assignment that ever sets it `True` — no code populates it from the module-level `_lane_instances` registry (the list that actually holds every `NFCGate`, lane and shared alike). This flag is not cosmetic: `shared_preload.py`'s `SharedPreloadCoordinator.clear_assigned()` reads it to decide whether "Happy Hare already had this spool assigned" was because a *per-lane* reader claimed it first (silent, expected) versus some other/unknown assignment (logged as a warning to the console). If the flag is always `False`, every `PRELOAD_CLEAR_ASSIGNED` reconciliation on a hybrid (per-lane + shared) install takes the "unknown assignment" warning branch even in the normal, expected per-lane-wins case — a false-positive warning on every such reconciliation, not a crash, but user-facing noise that undermines trust in the reconciliation logic.
