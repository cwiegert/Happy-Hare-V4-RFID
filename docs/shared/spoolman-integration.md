...
| QIDI | QIDI Box MIFARE Classic tags; requires `bambu_reads: True` (gates the implemented factory-key fallback attempt — no `pycryptodome` needed). The current code tries `FF FF FF FF FF FF`, confirmed from the community `BoxRFID-Touch` project. QIDI's own guide also identifies a sector-1 Key A that is documented but not yet implemented; see the [QIDI Box RFID reference](qidi-rfid-reference.md). |
